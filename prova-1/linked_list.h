...
structures::LinkedList<T> structures::LinkedList<T>::slicing(int start,
    ...
    auto first = before_index(start);
    list_slice.push_back(first->next()->data());
    ...
    while (i + step < stop) {
        ...
        list_slice.push_back(current->data());
        i += step;
    }
    return list_slice;
}
...
void structures::LinkedList<T>::append(structures::LinkedList<T> &list_add) {
    while (!list_add.empty()) {
        push_back(list_add.pop_front());
    }
}
...
structures::LinkedList< structures::LinkedList<T> * >
    ...
    auto even_index = new LinkedList();
    auto odd_index = new LinkedList();
    auto current = head;
    for (auto i = 0u; i < size(); i++) {
        if (i % 2 == 0) {
            even_index->push_back(current->data());
        } else {
            odd_index->push_back(current->data());
        }
        current = current->next();
    }
